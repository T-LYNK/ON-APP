...
export default function settings() {
  const items = [
      {
        name: 'Edit Account',
        url: '/(screens)/settings/editprofile'
      },
      {
        name : 'Edit Password'
      },
  ...
  return (
    <View style={{flex:1, justifyContent:'center', alignItems:'center', backgroundColor: 'black', }}>
       <View style={{ alignItems: 'center', padding: 10, alignSelf: 'flex-start', marginHorizontal: 100 }}>
        ...
        <View style={{  flex: 1 , justifyContent: 'center', alignItems: 'center', }}>
          <FlatList data={items} scrollEnabled={false} renderItem={( { item }) => (
            <View style={{ alignItems:'baseline', flexDirection: 'column', width: 350, margin:10, padding:5, backgroundColor: 'gray', borderRadius: 10 }}> 
              <Text style={{ fontSize:20, fontWeight: 'bold', justifyContent: 'flex-start', paddingHorizontal: 10, textAlign: 'left' , color: 'white'}}><Link href={'/(screens)/settings/editprofile'}>{item.name}</Link></Text>
